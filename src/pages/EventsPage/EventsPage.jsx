...
const EventsPage = () => {
  ...
  const fetchEventData = (name) => {
    firebase.getAllDocuments(name);
  };

  useEffect(() => {
    Promise.all([
      fetchEventData("events"),
      fetchEventData("pre-event"),
      fetchEventData("guest-talk"),
    ]);
  }, []);
      ...
      {isAboveLargeScreen ? (
        <>
          <motion.div className=" flex justify-around md:w-[70%] mx-auto mt-[44px] md:flex-row flex-col items-center md:gap-0 gap-3">
            <div>
              <Button
                text="Pre-Events"
                outline={eventType !== "pre-event"}
                onPress={() => setEventType("pre-event")}
              ></Button>
            </div>
            <div>
              <Button
                text="Events"
                outline={eventType !== "events"}
            ...
            <div>
              <Button
                text="Guest talks"
                outline={eventType !== "guest-talk"}
                onPress={() => setEventType("guest-talk")}
              ></Button>
            </div>
          </motion.div>
        ...
        <>
          <motion.div className=" flex justify-center items-center mb-[32px] mt-8 mx-8 ">
            <motion.div className="z-0 bg-blue h-[1.5px] basis-5/12"></motion.div>
            <motion.h1 className=" relative top-[50%] z-100  font-semibold text-blue sm:text-3xl ss:text-2xl text-xl min-w-max p-6 ">
              Pre-Events
            </motion.h1>
            <motion.div className="z-0 bg-blue h-[1.5px]  basis-5/12"></motion.div>
          </motion.div>
